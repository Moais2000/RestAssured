package API_TESTS_PET_STORE;

import io.restassured.http.ContentType;
import org.json.simple.JSONArray;
import org.json.simple.JSONObject;
import org.testng.annotations.DataProvider;
import org.testng.annotations.Test;

import static io.restassured.RestAssured.given;

public class POST_USER {

    @DataProvider(name = "User_Data")
    public Object[][] User_Data()
    {

        return new Object[][]{

                {25,"moamen200","moamen","ashraf","ashrafmoamen56@gmail.com","moamen123456","10122151",0},
        };
    }

    @Test(dataProvider = "User_Data")
    public void post_user(int ID, String username,String fname,String lname ,String email,String password,String phone,int userStatus)
    {

        JSONObject request = new JSONObject();
        request.put("id",ID);
        request.put("username",username);
        request.put("firstName",fname);
        request.put("lastName",lname);
        request.put("email",email);
        request.put("password",password);
        request.put("phone",phone);
        request.put("userStatus",userStatus);

        JSONArray obj=new JSONArray();
        obj.add(request);

        given().body(obj.toJSONString()).contentType(ContentType.JSON).
                when().post("https://petstore.swagger.io/v2/user/createWithList").then().statusCode(200);
    }
}
